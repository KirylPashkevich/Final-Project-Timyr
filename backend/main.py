...
def seed_database():
    ...
    if count == 0:
        seed_items = [
            ...
            ("Dota 2", 2, 52, 233, 1),
            ("R.E.P.O.", 15, 15, 2, 0),
            ("War thunder", 2, 28, 1, 0),
            ("PUBG", 2, 49, 1, 0),
        ]
        for name, sector, weight, qty, danger in seed_items:
            cursor.execute(
                "INSERT INTO items (name, storage_sector, weight, quantity, is_dangerous, image) VALUES (?, ?, ?, ?, ?, ?)",
                (name, sector, weight, qty, danger, "/static/img/default.jpg"),
            )
        conn.commit()
    conn.close()
...
async def create_item(
    ...
    cursor.execute('''
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    ''', (name, storage_sector, weight, quantity, 1 if is_dangerous else 0, image_url))
